...
tile_wall_cords = {
    -- walls that allow diagonal movement
    { 22, 5, 'WX'},
    { 21, 5, 'WX'},
    { 20, 5, 'AW'},
    { 20, 6, 'AD'},
    ...
    { 17,7, 'D'},
    { 17,8, 'D'},
    -- walls that block diagonal movement
    { 8, 5, 'QWEZXC'},
    { 9, 5, 'QWEZXC'},
    { 10, 5, 'QWEDC'},
    { 10, 6, 'QAZEDC'},
...
pajarito.init(tile_map)

-- this function needs the tile map dimensions to be set already
-- if it is called before, the walls are no added
pajarito.setWallsByList(tile_wall_cords)
...
-- callend once we are to go to the main menu
local function cleanUp()
    loveframes.RemoveAll()
    pajarito.clearNodeInfo()
    pajarito.clearWalls() -- if you are goin to use walls, you need to call this.
    SCENA_MANAGER.pop()
end
...
function drawWalls()
    for _, wall_pos in ipairs(tile_wall_cords) do
        local x = wall_pos[1]
        local y = wall_pos[2]
        local str = wall_pos[3]
        love.graphics.setColor(1,1,0,1)
        -- Note that string.find is not the faster way to do this
        if string.find(str, "W") then
            love.graphics.line(x*17+2,y*17,(x+1)*17-4,y*17)
        end
        if string.find(str, "S") or  string.find(str, "X") then
            love.graphics.line(x*17+2,(y+1)*17,(x+1)*17-4,(y+1)*17)
        end
        if string.find(str, "A") then
            love.graphics.line(x*17,y*17+2,x*17,(y+1)*17-4)
        end
        if string.find(str, "D") then
            love.graphics.line( (x+1)*17,y*17+2,(x+1)*17,(y+1)*17-4)
        end
        -- Diagonals
        
        if string.find(str, "E") then
            love.graphics.circle('fill',(x+1)*17,y*17,3)
        end
        if string.find(str, "Q") then
            love.graphics.circle('fill',x*17,y*17,3)
        end
        if string.find(str, "C") then
            love.graphics.circle('fill',(x+1)*17,(y+1)*17,3)
        end
        if string.find(str, "Z") then
            love.graphics.circle('fill',x*17,(y+1)*17,3)
        end
    end
...
function Main()
    ...
    function self.draw()
        ...
        love.graphics.setColor(1,1,1)
        love.graphics.print(tostring(m_ix)..', '..tostring(m_iy),17,270)
